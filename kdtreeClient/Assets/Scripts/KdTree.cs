...
public class KdTree<T> : IEnumerable<T>, IEnumerable where T : Component
{
    ...
    private void _add(KdNode newNode)
    {
        ...
        {
            _root = newNode;
            CubeBehaviour tmp = _root.component as CubeBehaviour;
            tmp.SetParent();
            return;
        }
        ...
        CubeBehaviour t = parent.component as CubeBehaviour;
        if (splitNew < splitParent)
        {
            parent.left = newNode; //go left
            t.SetLeft(newNode.component.transform.position);
        }
        else
        {
            parent.right = newNode; //go right
            t.SetRight(newNode.component.transform.position);
        }
    ...
    public T FindClosest(Vector3 position, out float nearest)
    {
        return _findClosest(position, out nearest);
    }
    ...
    public IEnumerable<T> FindClose(Vector3 position)
    {
        ...
        float nearest = 0f;
        _findClosest(position, out nearest, output);
        return output;
    }

    protected T _findClosest(Vector3 position, out float nearestDist, List<T> traversed = null)
    {
